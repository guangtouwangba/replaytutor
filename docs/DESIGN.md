...
- 已经使用 Codex、Claude Code、Kimi，希望复用原生 Agent 订阅与本地能力的用户。
...
- “用我已经登录的 Codex/Claude/Kimi 做 Tutor，不要求重复购买同一个模型的 API。”
...
6. **Agent 设置**：Codex、Claude Code、Kimi 和通用 Agent。
...
Codex 消费经过裁剪的 `TutorContext` 并返回合法 `TutorResponse`；无效证据引用被拒绝，Agent 失败不改变交易与指标真源。Claude Code 在第一条切片稳定后通过同一契约接入。
