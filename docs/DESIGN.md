...
提议注记（2026-08-03）：Chrome Companion 是独立 Side Panel 产品面，只提供本地连接、
Session 选择、受控上下文、Tutor 对话和 ReplayTutor 证据跳转。它不复制完整工作台，也不
读取或注入 TradingView 页面。产品范围与窄栏交互见
[Chrome Companion PRD](chrome-companion-prd.md)。
...
- 训练上下文工具栏提供全屏切换；原生全屏不可用时降级为应用内沉浸布局，`Esc` 退出。
- 自选、Tutor、证据、Playbook、品种详情和数据质量进入右侧可切换工具坞。
- 桌面端主图与右侧工具坞、左右双图/四图的两列提供可拖动分隔条；各层宽度
  独立保存在浏览器本地，方向键可微调，双击分隔条恢复默认。Chat 只保留单一对话面板，
  不再拆分历史侧栏与对话区。
...
- 支持单图、左右双图、上下双图和四图布局；每个窗格可独立选择
  ...
  `visible_at`。左右双图与四图共享一条可调列分隔线；活动窗格接收周期快捷键、缩放、
  复位与新绘图操作。
...
- 选中对象后的浮动操作条默认收起为对象名称和展开入口；只有用户主动展开时才显示样式、
  复制、模板、锁定、隐藏和删除，切换选中对象后重新收起，避免持续遮挡行情。
- 用户标注与 AI 建议分层展示。AI 建议默认 `proposed`，必须经 Inspector
  接受、拒绝或修改；修改与删除保留不可变原始对象和处置历史。
- 用户在 Chat 中明确要求“画、标出、添加”时，Tutor 可按当前活动窗格周期提出趋势线、
  水平支撑/压力、平行通道和支撑/压力区域；未指定数量时只保留 3–5 个关键对象。普通问答
  不主动创建图形。提案立即显示为紫色虚线，接受后可在图上拖动控制点并追加修订事件。
...
- 激活/选中态统一使用整圈边框、浅背景和文字/图标高亮，不使用左侧装饰色条；工作区边界、
  双列分隔线和按钮组分隔线等结构性边线继续保留。
...
Chat 面板采用单栏布局：只展示当前对话和固定在底部的输入区，不在工作台内展开
过去的线程列表。顶栏可新建对话；刷新后恢复当前对话。低于 1180px 时右侧面板改为覆盖式抽屉，
不压缩图表到不可用宽度。过去的线程仍按 Session 隔离保存，不进入其他回放会话的上下文。
