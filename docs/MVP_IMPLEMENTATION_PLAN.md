...
- 训练配置页选择 BTCUSDT/ETHUSDT、市场类型与历史范围后自动匹配覆盖足够的本地
  Snapshot；范围默认 30 天，也可选择 1 年。无匹配数据或覆盖不足时创建对应范围的
  Binance 公开 1m 行情后台任务，校验通过后生成 Snapshot；用户回到训练配置页再
  显式创建会话，后台完成不得强制页面跳转。
- 本地存在多个候选 Snapshot 时，显示版本 ID、覆盖区间、创建时间与质量并允许显式选择；
  自动匹配只提供默认值，创建请求必须携带用户最终选择的 `snapshot_id`。
