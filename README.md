...
> 当前阶段：MVP 1A 的 BTCUSDT 核心纵向闭环和本地试用硬化已落地，包括 Snapshot、逐 K 回放、计划门禁、模拟订单、Decimal 账本、趋势线/矩形/标记、AI 标注处置、证据回跳、确定性复盘、版本化 Playbook 规则检查、五维训练聚合与推荐、可恢复会话删除、SQLite 备份恢复、Codex Tutor 和隔离浏览器 E2E。Binance U 本位只读成交复盘作为独立切片保留。本项目不提供任何真实下单能力。
...
- A 股规则、其他市场和跨周期报告属于后续阶段。

本地试用前请运行 `make verify && make e2e`，并查看
[发布清单](docs/RELEASE_CHECKLIST.md) 的恢复演练与已知限制。
