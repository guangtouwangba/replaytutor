...
const evidenceDependentMetricKeys = new Set(["win_rate", "mfe", "mae", "r_multiple", "exit_efficiency"]);

function outcomeLabel(outcome: string) {
  if (outcome === "insufficient_evidence") return "证据不足";
  if (outcome.includes("good")) return "过程良好";
...
export function SessionReviewPage({ complete = false }: { readonly complete?: boolean }) {
  ...
  const hasInsufficientEvidence = data.process_outcome === "insufficient_evidence";
  ...
  return (
    <section className="page deterministic-review">
      <div className="review-hero">
        ...
        <span className={`outcome-badge ${data.process_outcome.includes("good") ? "good" : ""}`}>{outcomeLabel(data.process_outcome)}</span>
      </div>
      {hasInsufficientEvidence && (
        <div className="review-evidence-notice" role="status">
          <div><strong>本场没有足够的交易证据</strong><p>账户与成交事实仍保留；胜率、MFE/MAE、R 倍数和退出效率暂不评估，避免把“没有样本”误读成零分。</p></div>
          <Link className="secondary-action" to="/setup">开始一场有效训练</Link>
        </div>
      )}
      <div className="review-metrics">{data.metrics.map((metric) => {
        const isUnavailable = hasInsufficientEvidence && evidenceDependentMetricKeys.has(metric.key);
        return <article className={isUnavailable ? "is-unavailable" : ""} key={metric.key}><small>{metric.label}</small><strong>{isUnavailable ? "—" : metric.value}</strong><span>{isUnavailable ? "未评估" : metric.unit}</span></article>;
      })}</div>
