...
export function AnnotationInspector({
  ...
  return (
    <div className="dock-card annotation-inspector">
      ...
      <label>标注文字<input maxLength={200} onChange={(event) => setLabel(event.target.value)} value={label} /></label>
      {points.map((point, index) => (
        <div className="annotation-point-editor" key={`${point.time}-${index}`}>
          <label>UTC 时间<input onChange={(event) => setPoints((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, time: `${event.target.value}:00.000Z` } : item))} type="datetime-local" value={point.time.slice(0, 16)} /></label>
          <label>价格<input onChange={(event) => setPoints((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, price: event.target.value } : item))} step="any" type="number" value={point.price} /></label>
        </div>
      ))}
      <div className="annotation-actions">
        {annotation.layer === "ai" && disposition.state === "proposed" && (
          <>
        ...
        {visible && <button disabled={pending || !label.trim()} onClick={() => onAction("revised", label, points)} type="button">保存修改</button>}
        {visible && <button className="danger-action" disabled={pending} onClick={() => onAction("deleted")} type="button">删除</button>}
      </div>
      <details>
