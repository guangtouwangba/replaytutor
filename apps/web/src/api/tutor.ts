...
export async function discoverCodex(): Promise<AgentCapability> {
  return validated(await apiFetch(`${API_BASE_URL}/api/v1/agents/codex`), "AgentCapability");
}
...
export async function startTutor(
  ...
  return validated(
    await apiFetch(`${API_BASE_URL}/api/v1/sessions/${sessionId}/tutor`, {
      method: "POST",
...
export async function fetchTutorRun(runId: string): Promise<TutorRun> {
  return validated(
    await apiFetch(`${API_BASE_URL}/api/v1/tutor/runs/${runId}`),
    "TutorRun",
  );
}
...
export async function cancelTutorRun(runId: string): Promise<TutorRun> {
  return validated(
    await apiFetch(`${API_BASE_URL}/api/v1/tutor/runs/${runId}/cancel`, {
      method: "POST",
    }),
    "TutorRun",
  );
