...
export async function fetchPreferences(): Promise<LocalPreferences> {
  return checked(
    await fetch(`${API_BASE_URL}/api/v1/settings/preferences`),
    "LocalPreferences",
  );
}
...
export async function fetchMaintenance(): Promise<MaintenanceStatus> {
  return checked(
    await fetch(`${API_BASE_URL}/api/v1/maintenance`),
    "MaintenanceStatus",
  );
}
...
export async function createBackup(): Promise<BackupArtifact> {
  return checked(
    await fetch(`${API_BASE_URL}/api/v1/maintenance/backups`, { method: "POST" }),
    "BackupArtifact",
  );
}
...
export async function restoreBackup(backupId: string): Promise<BackupArtifact> {
  return checked(
    await fetch(`${API_BASE_URL}/api/v1/maintenance/backups/${backupId}/restore`, {
      method: "POST",
    }),
    "BackupArtifact",
  );
...
export async function cleanupAgentRuns(): Promise<CleanupResult> {
  return checked(
    await fetch(`${API_BASE_URL}/api/v1/maintenance/cleanup-agent-runs`, {
      method: "POST",
    }),
    "CleanupResult",
  );
