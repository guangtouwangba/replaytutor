...
def find_bar_index_at_or_after(parquet_path: Path, start_time: datetime) -> int:
    """Return the ordered index of the first bar opening at or after start_time."""
    with duckdb.connect(":memory:") as connection:
        row = connection.execute(
            """
            SELECT count(*)
