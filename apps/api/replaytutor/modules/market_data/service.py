...
from replaytutor.storage.duckdb_query import count_bars, query_bar_slice, query_bars
...
class MarketDataService:
    ...
    async def download_binance(
        ...
        bars = await BinancePublicAdapter().fetch_klines(symbol, start_time, end_time)
        instrument = crypto_instrument(symbol)
        return self.commit_snapshot(
            instrument=instrument,
            bars=bars,
            source_id=f"binance-public:{symbol}:1m",
            source_kind="binance_public",
            provenance={
...
def crypto_instrument(symbol: str) -> NormalizedInstrument:
    ...
    return NormalizedInstrument(
        ...
        price_scale=2,
        market_rule_set_id="crypto_spot_v1",
    )
