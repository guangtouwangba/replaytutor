...
@router.get("/sessions/{session_id}/bars", response_model=BarListResponse)
...
@router.post("/sessions/{session_id}/orders/amend", response_model=OrderResult)
def amend_order(
    request: Request,
    session_id: str,
    ...
    try:
        return ExecutionService(service(request)).amend_order(session_id, payload)
    except (TrainingSessionError, MarketDataError, RuleViolation) as error:
        raise translate(error) from error
